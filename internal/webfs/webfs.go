...
// The SPA build (internal/webfs/dist) is generated by `npm run build` and is
// NOT committed; CI builds it before `go build`. When it is absent (a bare
// `go build` without building the frontend), serve a small placeholder instead
// of crashing.
func buildIndex(sub fs.FS, basePath string) []byte {
	raw, err := fs.ReadFile(sub, "index.html")
	if err != nil {
		return []byte("<!doctype html><meta charset=utf-8><title>Imugi Panel</title>" +
			"<body style=\"font-family:sans-serif;background:#0c1310;color:#eee;padding:2rem\">" +
			"<h1>Imugi Panel</h1><p>Web UI not built. Run <code>make build-web</code> " +
